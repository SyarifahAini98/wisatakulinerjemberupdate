...
<?php
	$query=mysql_query("SELECT * FROM berita where kategori='wisata'")or die('perintah sql salah');
	$sql = "SELECT count(*) AS jumlah FROM berita where kategori='wisata'";
	...
	echo'<tr><td><center>Tidak ada data!</center></td></tr>';
		...
		$page_query = mysql_query("SELECT COUNT(*) FROM berita where kategori='wisata'");
		$pages = ceil(mysql_result($page_query, 0) / $per_page);
		$page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
		$start = ($page - 1) * $per_page;
		$query="select judul,gambar,isi,id_berita from berita where kategori='wisata' order by id_berita limit $start, $per_page";

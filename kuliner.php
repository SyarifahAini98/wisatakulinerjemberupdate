...
<?php
	$per_page = 5;
	$page_query = mysql_query("SELECT COUNT(*) FROM berita where kategori='kuliner'");
	$pages = ceil(mysql_result($page_query, 0) / $per_page);
	$page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
	$start = ($page - 1) * $per_page;
	$query="select id_berita,judul,tgl_buat,tgl_update,gambar,isi,kategori,kunjungan from berita where kategori='kuliner' order by kunjungan DESC limit $start, $per_page";

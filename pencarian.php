...
<?php
	if($_GET['cari_teks']<>''){
		...
		$page_query = mysqli_query($koneksi,"SELECT COUNT(*) FROM berita where ((judul like '%$cari_teks%') or (isi like '%$cari_teks%'))");
		$pages = ceil(mysqli_result($page_query, 0) / $per_page);
		
		$page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
		$start = ($page - 1) * $per_page;
		$query="select id_berita,judul,tgl_buat,tgl_update,gambar,isi,kategori,kunjungan from berita where ((judul like '%$cari_teks%') or (isi like '%$cari_teks%')) order by kunjungan DESC limit $start, $per_page";

...
<center>
<table border="0">
	<tr><td colspan="2" align="right"><form method="get" action="pencarianwisata.php"><input type="text" name="cari_teks" value="<?php echo $_GET[cari_teks];?>"><input type="submit" name="cari" value="Cari" ></form></td></tr>
</table>
...
<?php
	if($_GET['cari_teks']<>''){
		...
		$page_query = mysql_query("SELECT COUNT(*) FROM berita where ((judul like '%$cari_teks%') or (isi like '%$cari_teks%')) and kategori='wisata'");
		$pages = ceil(mysql_result($page_query, 0) / $per_page);
		
		$page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
		$start = ($page - 1) * $per_page;
		$query="select id_berita,judul,tgl_buat,tgl_update,gambar,isi,kategori,kunjungan from berita where ((judul like '%$cari_teks%') or (isi like '%$cari_teks%')) and kategori='wisata' order by kunjungan DESC limit $start, $per_page";
